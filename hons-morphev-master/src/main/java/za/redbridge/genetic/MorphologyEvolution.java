 ...
 */
public class MorphologyEvolution extends BasicTraining implements
		MultiThreadable {
	...
	public MorphologyEvolution(final MethodFactory phenotypeFactory, final CalculateScore calculateScore, final int populationSize) {
		...
		for (int i = 0; i < population.getPopulationSize(); i++) {
			/*final MLEncodable chromosomeNetwork = (MLEncodable) phenotypeFactory
					.factor();
			final MorphGenome genome = new MorphGenome(chromosomeNetwork);*/
			final MorphGenome genome = (MorphGenome) phenotypeFactory.factor();
			defaultSpecies.add(genome);
		}
		defaultSpecies.setLeader(defaultSpecies.getMembers().get(0));

		population.setGenomeFactory(new MorphGenomeFactory(phenotypeFactory,
				population));
		
		// create the trainer
		this.genetic = new MorphologyEvolutionHelper(population,
				calculateScore);
